...
def mock_server(monkeypatch, tmp_path):
    ...
    server.storage.active_repo_path = str(tmp_path / "mock-repo")
...
def test_init_memento_with_custom_path(mock_server, tmp_path):
    ...
    expected_file = mock_server.storage.repo_mementos_file(custom_path)
...
def test_repo_key(mock_server):
    ...
    key = mock_server.storage.repo_key("/home/user/my_cool_project")
...
def test_memento_lifecycle_global_to_repo(mock_server):
    ...
    # 1. Create (Save)
    response = mock_server.handle_save_memento({
        "scope": "global",
    ...
    # Extract ID
    mementos = mock_server.storage.load_global_mementos()
    ...
    # 3. Move (Global -> Repo)
    move_response = mock_server.handle_move_memento({"id": memento_id, "target_scope": "repo"})
    assert "Memento moved" in move_response
    assert "global -> repo" in move_response

    assert len(mock_server.storage.load_global_mementos()) == 0
    assert len(mock_server.storage.load_repo_mementos()) == 1

    # 4. Delete
    delete_response = mock_server.handle_delete_memento({"id": memento_id})
    assert "Memento deleted" in delete_response

    assert len(mock_server.storage.load_repo_mementos()) == 0


def test_is_active(mock_server):
    """Ensure date comparisons for expiry work robustly."""
    is_active = mock_server.repo.is_active
    assert is_active({"expires": "never"}, "2026-04-09") is True
    assert is_active({"text": "test"}, "2026-04-09") is True
    assert is_active({"expires": "2026-04-10"}, "2026-04-09") is True
    assert is_active({"expires": "2026-04-08"}, "2026-04-09") is False
...
def test_load_json_flat_array_migration(mock_server, tmp_path):
    ...
    data = mock_server.storage.load_json_file(fpath, "migration_test")
    assert len(data) == 1
    assert data[0]["text"] == "legacy text"

    mock_server.storage.save_json_file(fpath, data)
...
def test_save_conversation_creates_attachment_files(mock_server):
    ...
    mementos = mock_server.storage.load_repo_mementos()
    assert len(mementos) == 1
    memento = mementos[0]
    assert memento["attachments"].endswith("_attachments")

    dir_name = memento["attachments"]
    attachments_dir = mock_server.storage.get_attachments_dir(memento, dir_name)
...
def test_save_memento_attachments_adds_files_to_existing_memento(mock_server, tmp_path):
    ...
    memento = mock_server.storage.load_repo_mementos()[0]
    ...
    updated_memento = mock_server.storage.load_repo_mementos()[0]
    dir_name = updated_memento["attachments"]
    attachments_dir = mock_server.storage.get_attachments_dir(updated_memento, dir_name)
...
def test_get_memento_attachments_returns_markdown(mock_server):
    ...
    memento = mock_server.storage.load_global_mementos()[0]
